"""
rolling_monitor_hennelly.py - Project script (example).
...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_timeseries_hennelly.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "rolling_metrics_hennelly.csv"
...
def main() -> None:
    ...
    # STEP 3.3.1: DEFINE ROLLING STD FOR REQUESTS
    requests_rolling_std_recipe: pl.Expr = (
        pl.col("requests").rolling_std(WINDOW_SIZE).alias("requests_rolling_std")
    )

    # STEP 3.3.2: DEFINE ROLLING STD FOR ERRORS
    errors_rolling_std_recipe: pl.Expr = (
        pl.col("errors").rolling_std(WINDOW_SIZE).alias("errors_rolling_std")
    )

    # STEP 3.3.3: DEFINE ROLLING STD FOR LATENCY
    latency_rolling_std_recipe: pl.Expr = (
        pl.col("total_latency_ms").rolling_std(WINDOW_SIZE).alias("latency_rolling_std")
    )
    # ----------------------------------------------------
    ...
    df_with_rolling = df.with_columns(
        [
            ...
            requests_rolling_std_recipe,  # new add
            errors_rolling_std_recipe,  # new add
            latency_rolling_std_recipe,  # new add
        ]
    )

    LOG.info("Computed rolling mean signals")

    # STEP 3.4.1: FLAG ANOMALIES (value exceeds mean + 2 std deviations)
    df_with_rolling = df_with_rolling.with_columns(
        [
            (
                pl.col("errors")
                > pl.col("errors_rolling_mean") + 2 * pl.col("errors_rolling_std")
            ).alias("error_spike_flag"),
            (
                pl.col("total_latency_ms")
                > pl.col("latency_rolling_mean") + 2 * pl.col("latency_rolling_std")
            ).alias("latency_spike_flag"),
        ]
    )

    LOG.info(f"Flagged {df_with_rolling['error_spike_flag'].sum()} error spikes")
    LOG.info(f"Flagged {df_with_rolling['latency_spike_flag'].sum()} latency spikes")
    # ----------------------------------------------------
